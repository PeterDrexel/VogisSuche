...
class VogisSucheDockWidget(QDockWidget, FORM_CLASS):
	...
	def sucheLeChanged(self):
		...
		# Testserver
		# LuceneSolrUrl = 'http://cnvbrwgdit2.net.vlr.gv.at/solr/geoland/select?q='+urllib.parse.quote(Suchtext)+'&qf=phonetic%5E10.0%20textsuggest%5E30.0%20textng%5E50.0%20extrasearch&wt=json&qt=ac&sort=score%20desc%2Cpopularity%20desc&defType=edismax&q.op=AND&v.template=suggest'
		...
		if len(Suchtext)==0:
			...
			for Serveraufrufe in range(20):
				try:
					...
					self.ergebnisTw.horizontalHeader().setMinimumSectionSize(0)		# Seit QT 5.11 noetig
					self.ergebnisTw.clearSpans()
		...
		# self.ergebnisTw.sortItems(7, Qt.AscendingOrder)
